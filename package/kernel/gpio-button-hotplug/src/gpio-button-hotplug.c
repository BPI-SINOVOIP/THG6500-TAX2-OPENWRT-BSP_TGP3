...
irqreturn_t (*tr6560_gpio_irq_deal)(unsigned int irq, int *devid);
...
static irqreturn_t button_handle_irq(int irq, void *_bdata)
{
	...
	if (tr6560_gpio_irq_deal){
		tr6560_gpio_irq_deal(irq, &(bdata->b->gpio));
	}
...
gpio_keys_get_devtree_pdata(struct device *dev)
{
	...
	for_each_child_of_node(node, pp) {
		...
		if (of_property_read_u32_array(pp, "gpios", array, 3) < 0) {
			printk("failed to find gpios offset\n");
			button->gpio = -ENOENT; /* mark this as device-tree */
		}
		else{
			button->gpio = array[1];
		...
		struct gpio_keys_button_dev **_bdev, int polled)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if (gpio_is_valid(button->gpio) && (polled > 0)) {
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if (!button->irq) {
			...
			irqflags |= IRQF_TRIGGER_HIGH | IRQF_NO_THREAD | IRQF_SHARED;
		} else {
		...
		if (ret < 0) {
			...
			tr6560_gpio_irq_deal = __symbol_get("tri_gpio_irq_deal_super");
		}
	}
