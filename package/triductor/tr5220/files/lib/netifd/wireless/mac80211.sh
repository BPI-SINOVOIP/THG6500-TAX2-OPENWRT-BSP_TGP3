...
drv_mac80211_setup() {
	...
	if [ -n "$rts" ]; then
		# rts alreday set by the preceding line
		:
	else
		# initiate a dummy RTS setting to trigger TR5220 driver to fall-back edca_prot_mode_opt(iwpriv wlanl alg "edca_prot_mode_opt 2 1")
		iw phy "$phy" set rts 3333
	fi
